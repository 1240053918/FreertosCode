...
SD_Error SD_WriteBlock(uint8_t *buf,long long addr,  uint16_t blksize)
{
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	{
		SD_DMA_Config((uint32_t*)buf,blksize,DMA_DIR_PeripheralDST);//SDIO DMA配置
   		TransferError=SD_OK;
		StopCondition=0;			//单块写,不需要发送停止传输指令 
		TransferEnd=0;				//传输结束标置位，在中断服务置1
...
SD_Error errorstatus = SD_OK;
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	{					  
		...
		{
			SD_DMA_Config((uint32_t*)buf,nblks*blksize,DMA_DIR_PeripheralDST);//SDIO DMA配置
	   		TransferError=SD_OK;
...
void SD_DMA_Config(uint32_t*mbuf,uint32_t bufsize,uint32_t DMA_DIR)
{		 
